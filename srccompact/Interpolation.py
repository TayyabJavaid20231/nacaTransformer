...
def get_KNNs(xq, xb, yb, deltax, xmin, ymin, k_min, gpu_id):
	...
	uni, count = np.unique(source2targetmapping, return_counts = True, axis = 0)
	...
	for i in range(np.shape(xq)[0]):
		...
		k = np.array(calculate_K(source2targetmapping, xq[i]))
		if np.shape(k)[-1] > sizek:
			sizek = np.shape(k)[-1]
		if np.shape(k)[-1] <= k_min:			
			index[i] = np.concatenate((idx[i].reshape(1,-1), np.zeros((1,max_count-k_min))), axis = 1) 
		else:
			index[i] = np.concatenate((k.reshape(1,-1), np.zeros((1,max_count-np.shape(k)[-1]))), axis = 1)

	distance = np.where(index == 0, 0, dist[index])

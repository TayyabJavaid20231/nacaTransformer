...
def plot(Z, name):
	...
	#X = xq[:,0].reshape(nx,ny)
	#Y = xq[:,1].reshape(nx,ny)
	Z = Z.T
	fig,ax=plt.subplots(1,1)
	cp = ax.contourf(Z)
	fig.colorbar(cp) # Add a colorbar to a plot
	plt.savefig('Database/' + name + '.png')
	#plt.show()
...
def copyfiles(vtu_file, stl_file):
    VTUDirectory = '/local/disk1/nacaFOAM/airfoilMNIST-raw/vtu/'
    STLDirectory = '/local/disk1/nacaFOAM/airfoilMNIST-raw/stl/'
	#change here
    #vtu_file = files[i][:-1]
    VTUFile = VTUDirectory + vtu_file
    vtu_dir = os.path.join('vtu', vtu_file)
    shutil.copy(VTUFile, vtu_dir)

    #stl_file = vtu_file[:vtu_file.rfind("_")] + '.stl'
    STLFile = STLDirectory + stl_file
    stl_dir = os.path.join('stl', stl_file)
    shutil.copy(STLFile, stl_dir)


def deletefiles(vtu_file, stl_file):
    stl_dir = os.path.join('stl', stl_file)
    os.remove(stl_dir)
    vtu_dir = os.path.join('vtu', vtu_file)
    os.remove(vtu_dir)
...
def generate_tfds_dataset(config: ConfigDict):
    ...
    """
    #config.preprocess.writedir = 'tffiles'
    #config.preprocess.readdir = ''
    #config.preprocess.nsamples = 5
    ...
    for i in tqdm(range(n_files_test), desc='Test split', position=0):
        ...
        with tf.io.TFRecordWriter(file_dir) as writer:
            ...
            for sample in tqdm(batch, desc='Shards', position=1,
                ...
                try:
                    x, y = vtu2structured(vtu_dir, stl_dir, mach)
                    example = create_tfExample(x, y, sim_config)
                    writer.write(example.SerializeToString())
